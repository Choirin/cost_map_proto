...
int main(int argc, char *argv[]) {
  ...
  scan_buffer.update(0.0, (1 / angles->array().cos()).matrix(),
                     Eigen::Vector2d(0.0, 0.0), M_PI / 2.0);
  scan_buffer.update(0.0, (1 / angles->array().cos()).matrix(),
                     Eigen::Vector2d(-5.0, 0.0), 0.0);
  scan_buffer.update(0.0, (1 / angles->array().cos()).matrix(),
                     Eigen::Vector2d(-15.0, 0.0), 0.0);
  scan_buffer.update(0.0, (1 / angles->array().cos()).matrix(),
                     Eigen::Vector2d(-10.0, 0.0), 0.0);
  // scan_buffer.update(0.0, (1 / angles->array().cos()).matrix(),
  // Eigen::Vector2d(-5.0, 0.0), 0.0);
  ...
  if (cost_map.crop("cost", center - half_size, 2 * half_size, cropped)) {
    Eigen::Array<float, Eigen::Dynamic, Eigen::Dynamic> array = cropped.array();
